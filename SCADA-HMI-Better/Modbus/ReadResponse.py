...
def repackReadResponse(bytes : bytearray):
    base = ModbusBase(int.from_bytes(bytes[7:8], byteorder="big", signed=False),
                      int.from_bytes(bytes[8:9], byteorder="big", signed=False))
    """
    base.setProtocolID(protocolID)
    base.setLength(length)
    """
    readResponse = ModbusReadReasponse(base,int.from_bytes(bytes[9:10],byteorder="big", signed=False),
