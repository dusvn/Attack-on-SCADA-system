...
def repack(ReadRequest: ModbusReadRequest) -> bytearray:
    ...
    request[0:2] = socket.htons(ReadRequest.TransactionID)
    request[2:4] = socket.htons(ReadRequest.ProtocolID)
    request[4:6] = socket.htons(ReadRequest.Length)
    request[6] = ReadRequest.UnitID
    request[7] = ReadRequest.FunctionCode
    request[8:10] =  socket.htons(ReadRequest.StartAddress)
    request[10:12] = socket.htons(ReadRequest.Quantity)
